    ...
    <page-main>
        <el-table v-loading="loading" :data="filterTableData" style="width: 100%;">
            <el-table-column align="right">
                <template #header>
                    <el-input v-model="search" size="small" placeholder="搜索主题名" @change="searchTag" />
                </template>
                <template #default="scope">
                    <el-button size="small" @click="handleEdit(scope.$index, scope.row)">修改</el-button>
                    <el-popconfirm confirm-button-text="确定" cancel-button-text="取消" :icon="InfoFilled" icon-color="#626AEF" title="这可能是用户的精心设计，确定要删除吗?" @confirm="handleDelete(scope.$index, scope.row)" @cancel="cancelEvent">
                        <template #reference>
            ...
            <el-table-column label="导航栏激活背景颜色" prop="--el-menu-active-color" />
            ...
            <el-table-column label="内容描述文字颜色" prop="--content-descript-text-color" />
            <el-table-column label="评论区背景颜色" prop="--comment-bg-color" />
            <el-table-column label="评论区描述文字颜色" prop="--comment-text-color" />
            <el-table-column label="评论区内容文字颜色" prop="--comment-content-color" />
            <el-table-column label="底部描述文字颜色" prop="--article-describe-text-color" />
...
export default {
    ...
    computed: {
        filterTableData() {
            return this.tableData.filter(data => !this.search || data.name.toLowerCase().includes(this.search.toLowerCase())).slice(this.pageSize * (this.currentPage - 1), this.pageSize * (this.currentPage - 1) + this.pageSize)
        }
    },
    mounted() {
        this.getThemes()
    ...
    methods: {
        // 更新操作
        updateDate() {
            api.post(apiUrl.updateTheme, this.form).then(() => {
        ...
        getThemes() {
            this.loading = true
            api.get(apiUrl.themes).then(response => {
